...
void svc_handler_c(uint32_t EXE_RETURN, uint32_t msp)
{
	printf("[SVC Handler] LR: 0x%X\r\n", (unsigned int)EXE_RETURN);
	...
	if (EXE_RETURN & ((uint32_t) 1<<2)) //Test bit 2 of EXC_RETURN
	{
	...
	if (svc_num == 0xA)
		//return r0 + r1
		*(stack_frame_ptr)=*(stack_frame_ptr)+*(stack_frame_ptr+1);
	else
		//return 0
		*(stack_frame_ptr)=0;
}
